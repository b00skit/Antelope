    ...
    if (roster.roster_setup_json) {
        try {
            ...
            if (isForumFilterActive) {
                ...
                if (!cachedForumData?.data) {
                    return { error: 'Forum data has not been synced. Please go to Sync Management.' };
                }

                const forumData = cachedForumData.data as any;
                includedUsernames = new Set(forumData.includedUsernames || []);
                excludedUsernames = new Set(forumData.excludedUsernames || []);

                const mapSource = forumData.usernameToGroupsMap;
                if (mapSource instanceof Map) {
                    usernameToGroupsMap = new Map(mapSource as Map<string, number[]>);
                } else if (mapSource && typeof mapSource === 'object') {
                    usernameToGroupsMap = new Map(
                        Object.entries(mapSource as Record<string, number[]>).map(([username, groups]) => [
                            username,
