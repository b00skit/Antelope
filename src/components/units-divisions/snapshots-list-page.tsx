...
import { AlertTriangle, Loader2, Eye, Trash2, Building, Building2 } from 'lucide-react';
...
export function SnapshotsListPage() {
    ...
    useEffect(() => {
        const fetchSnapshots = async () => {
            ...
            setError(null);
            try {
                const res = await fetch('/api/units-divisions/snapshots');
                if (!res.ok) {
    ...
    const handleDelete = async (snapshot: Snapshot) => {
        try {
            const res = await fetch(`/api/units-divisions/snapshots/${snapshot.type}-${snapshot.id}`, { method: 'DELETE' });
            if (!res.ok) {
                const data = await res.json();
                throw new Error(data.error || 'Failed to delete snapshot.');
            }
            toast({ title: 'Success', description: 'Snapshot deleted successfully.' });
            setSnapshots(prev => prev.filter(s => s.id !== snapshot.id || s.type !== snapshot.type));
        } catch (err: any) {
    ...
    return (
        <div className="container mx-auto p-4 md:p-6 lg:p-8">
            <PageHeader
                title="Organizational Snapshots"
                description="View historical, point-in-time captures of your units and details."
            />
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>Source Unit/Detail</TableHead>
                        ...
                        <TableBody>
                            {isLoading ? (
                                ...
                                snapshots.map(snapshot => (
                                    <TableRow key={`${snapshot.type}-${snapshot.id}`}>
                                        <TableCell className="font-medium">{snapshot.name}</TableCell>
                                        <TableCell>
                                            <Badge variant="secondary" className="flex items-center gap-2 w-fit">
                                                {snapshot.type === 'cat2' ? <Building2 /> : <Building />}
                                        ...
                                        <TableCell>
                                            <div className="flex gap-2">
                                                <Button variant="outline" size="icon" asChild>
                                                    <Link href={`/units-divisions/snapshots/${snapshot.type}-${snapshot.id}`}>
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            ...
                                                            <AlertDialogFooter>
                                                                ...
                                                                <AlertDialogAction onClick={() => handleDelete(snapshot)}>
                                                                    Yes, Delete
                                                                </AlertDialogAction>
                                                            </AlertDialogFooter>
                                                        </AlertDialogContent>
